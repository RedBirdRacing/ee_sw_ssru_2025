...
MCP2515 mcp2515_1(CS_CAN_1);
MCP2515 mcp2515_2(CS_CAN_2);
...
void sendCAN_0x750() {
  uint8_t data[8] = {0};
  readPotentiometerScaled(APPS_SENSOR_1, &data[0], &data[1]);
  readPotentiometerScaled(APPS_SENSOR_2, &data[2], &data[3]);
  int8_t accel[3], gyro[3];
  readMPU6050Scaled(MPU6050_ADDR_1, accel, gyro);
  data[4] = accel[0]; data[5] = accel[1]; data[6] = accel[2]; data[7] = gyro[0];
...
void sendCAN_0x752() {
  uint8_t data[8] = {0};
  readBrakeTempScaled(BRAKE_TEMP_SENSOR_1, &data[0], &data[1]);
  readBrakeTempScaled(BRAKE_TEMP_SENSOR_2, &data[2], &data[3]);
  readDS18B20Scaled(&data[4], &data[5]);
  readPumpThermistorScaled(&data[6], &data[7]);
